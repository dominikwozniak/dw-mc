...
import { Duration, Effect, Fiber, FileSystem, Layer, PlatformError } from "effect"
import { TestClock } from "effect/testing"
import type { ChildProcess } from "effect/unstable/process"

import type { Launcher } from "#adapters/config.ts"
import { builtInLauncher } from "#adapters/config.ts"
import { builtinFindings, builtinReview, codexReview, promptReview, steeredSession } from "#adapters/runner.ts"
...
describe("the prompt runner on Codex", () => {
  ...
  /** A machine with a `codex` that prints `stdout`, and a temporary file to hand it the schema in. */
  const codex = (options: {
    readonly spawned: Array<ChildProcess.StandardCommand>
    readonly stdout?: string | undefined
    ...
    Layer.mergeAll(
      claude({ spawned: options.spawned, stdout: options.stdout }),
      FileSystem.layerNoop({
        makeTempFileScoped: () => Effect.succeed("/tmp/dw-mc-findings-1.json"),
        writeFileString: (path, contents) => Effect.sync(() => options.written.push([path, contents]))
      })
    )

  const reviewing = (over: { readonly model?: string | null } = {}) =>
    codexReview({
      launcher,
      directory: "/worktree",
      prompt,
  ...
  it.effect("runs the same prompt against the same schema, and hands the findings on", () => {
    ...
    return Effect.gen(function* () {
      const reported = yield* reviewing()
      ...
      assert.strictEqual(spawned[0]?.options.cwd, "/worktree")
    }).pipe(Effect.provide(codex({ spawned, stdout: events([found]), written })))
  })
  ...
  it.effect("runs the prompt on the model the repository configured", () => {
    ...
    return Effect.gen(function* () {
      yield* reviewing({ model: "gpt-5.4-codex" })
  ...
  it.effect("takes the last thing the run said, because a schema lets it answer more than once", () => {
    ...
    }).pipe(Effect.provide(codex({ spawned, stdout: events([{ verdict: "clean", findings: [] }, found]), written })))
  })
  ...
  it.effect("a run that answered nothing is a failure, never a clean verdict", () => {
    ...
    return Effect.gen(function* () {
      const error = yield* Effect.flip(reviewing())

      assert.strictEqual(error._tag, "RunnerFailed")
      assert.include(error.message, "no structured output")
    }).pipe(Effect.provide(codex({ spawned, stdout: events([]), written })))
  })
  ...
  it.effect("a codex that exits non-zero is a failure in the name of codex", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        Layer.mergeAll(
          claude({ spawned, stderr: "Not logged in. Run codex login.\n", exitCode: 1 }),
          FileSystem.layerNoop({
            makeTempFileScoped: () => Effect.succeed("/tmp/dw-mc-findings-1.json"),
            writeFileString: (path, contents) => Effect.sync(() => written.push([path, contents]))
          })
        )
      )
    )
