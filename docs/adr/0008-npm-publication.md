...
changesets holds the version and the changelog, and nothing gates a pull request on carrying one: a changeset is the decision that something is worth releasing, not a condition of merging. A merge to `main` with changesets waiting opens a version pull request, and merging that one publishes. Publication is authorised by one npm token, held as a repository secret and scoped to this package alone, and the workflow asks sigstore for a provenance attestation as it publishes, so a release still says which commit and which workflow built it. It runs the whole `pnpm check` chain first, because [ADR 0005](./0005-quality-toolchain.md)'s gate deliberately skips the push a squash merge makes, and this is the one moment an artifact leaves the machine.
...
- Trusted publishing over OIDC, which holds no secret at all: not taken, and the one worth coming back to. npm binds a trusted publisher only to a package that already exists, so it cannot authorise the release that would create the package, and this account's second factor is a code mailed to it, which no workflow can read. A dated token scoped to one package covers both, and moving later is a setting on npmjs.com and one secret deleted. What the token must not cost is the attestation: `pnpm-workspace.yaml` sets `trustPolicy: no-downgrade`, so this repository already refuses dependencies that publish weaker than they used to, and publishing without provenance would be asking of others what it does not do itself.

## Consequences

- The token is the one credential this repository holds, and it expires. A release that fails on a `403` is a token to reissue and a secret to replace, not a bug in the workflow.
- `0.1.0` was published by hand, because until the package existed there was nothing for a token to be scoped to and nothing for a trusted publisher to bind to. Claiming the name with a throwaway version first would have left a version on the registry that can only ever be deprecated.
