...
**A prompt pays for its colour.** Effect's prompts count the rows they must erase from the length of what they drew, escape sequences included, so a coloured row erases a line above itself on every keypress unless the row is shorter by what the colour costs. A prompt's rows therefore say the bucket in one coloured cell and leave the rest of the row plain, and the room they are cut to is short by that cost.
