# A configuration file this version cannot read stops every command, and names what to change
...
therefore breaks every command on a machine whose file still spells it — `dw-mc init` included, because
it reads before it writes. That is the intended outcome: the alternative to stopping is running, and a
review that runs under settings I no longer have is worse than a command that will not start.
