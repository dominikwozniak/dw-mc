...
| fixes               | Report only. `dw-mc fix <pr>` opens a MultiSelect of findings, takes an optional note per finding, then launches an interactive `claude` in a fresh worktree with the selected findings as JSON in the prompt. It refuses where the head moved since the run, so no session fixes a finding at a line that has gone. I steer and push. The session commits only where `fix.commits` or `--commit` says so; the tool itself never commits and never pushes. The re-review is a new review run. Inside an open session, `/dw-mc findings <pr>` prints the same JSON.                          |
